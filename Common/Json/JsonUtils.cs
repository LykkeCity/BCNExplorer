...
namespace Common.Json
{
    public static class JsonUtils
    {
        ...
        public static object DeserializeJsonType(this Type type, string value)
        {
            if (type == typeof(string))
            {
                if (value.IsJsonValueNull())
                    return null;

                return value.CleanUpJsonValue();
            }


            if (type.IsEnum)
            {
                value = value.CleanUpJsonValue();

                if (value.IsOnlyDigits())
                {
                    var number = int.Parse(value);
                    value = Enum.GetName(type, number);
                }
